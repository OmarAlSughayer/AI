...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        # handeling the edge case of a capured ghost
        if noisyDistance == None:
            # give a probability of 1.0 (i.e. certainty) to the ghost being
            # in the jail position 
            allPossible[self.getJailPosition()] = 1.0

        # do the whole updating shinanegans if the ghost was not captured which might
        # save a tiny bit of computing power
        # TODO: will this work for more than one ghost?, also doesn't look pretty 
        else: 
            # loop over all legal positions
            for p in self.legalPositions:
                trueDistance = util.manhattanDistance(p, pacmanPosition)

                # if P(noisyDistance | TrueDistance) > 0.0 
                # (i.e. the ghost could be here with probability > 0.0)
                if emissionModel[trueDistance] > 0:
                    # update allPossible[p]
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for oldPos in self.legalPositions:
            ...
            # loop over all the new positions' distributions
            for i in newPosDist.items():
                # extract the new position and its distrubtion
                (newPos, newProb) = i
                # update allPossible
                allPossible[newPos] += newProb*self.beliefs[oldPos]

        # update your beliefs to match these of the 21st century
        self.beliefs = allPossible
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        particle could be located.

        IMPORTANT: Particles should be distributed across positions somewhat evenly
        in order to ensure the prior is as uniform as possible. Specifically, there
        should never be a position with two particles as long as there is some
        position with zero particles. Similarly, there should never be a position
        with three particles as long as there is one with less than two, and so on.
        Beyond this, the exact details of how you allocate particles to available
        positions are up to you.
        ...
        # initeliaze particles
        self.particles = []

        # get the number of particles to be created
        num = self.numParticles

        # the pool of positions to choose from, we pick a random position from this pool
        # without replacement until there are not more position then we fill it again
        # with the same initial positions to ensure unifirmity in the given manner
        # we deep copy the list to avoid removing positions from self.legalPositions
        pool = list(self.legalPositions)

        # while more particles remain to be picked
        while num > 0:
            ...
            if len(pool) == 0:
                ...
                randomPosition = random.choice(pool)
                # remove the random particle from the pool
                pool.remove(randomPosition)

                # add the random position to the particles
                self.particles.append(randomPosition)
                # decreas num
                num -= 1
    ...
    def observe(self, observation, gameState):
        ...
        # handle the case when a ghost have been captured
        if noisyDistance == None:
            # self.particles = [jail, jail, jail, ..., jail] 
            self.particles = [self.getJailPosition()]*self.numParticles
        else:
            # get the belief distrubtion
            beliefDists = self.getBeliefDistribution()

            # loop over all 
            for p in beliefDists:
                # get the true distance
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                # update the weights
                beliefDists[p] *= emissionModel[trueDistance]


            # if all weights are zero reinitialize
            if beliefDists.totalCount() == 0.0:
                self.initializeUniformly(gameState)
            else:
                # normalize
                beliefDists.normalize()
                # get all the elements of weights
                items = sorted(beliefDists.items())
                # seperate the distrubtions (probabilities) from the values (positions)
                distribution = [i[1] for i in items]
                values = [i[0] for i in items]

                # create a new list of particles by sampling numParticles values 
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.particles:
            # get the new positions distributions
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            # include a sample
    ...
    def getBeliefDistribution(self):
        ...
        # create the new belief distrubituion 
        newBeliefDistribution = util.Counter();

        # add the number of particles in every position to the new belief distribution
        # remember that counter[_element_never_added] = 0
        for p in self.particles:
            newBeliefDistribution[p] += 1.0

        # normalize the new bliefs
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        # find the possible permutatoins of places using itertools
        permutatoins = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
        ...
        # with the same initial positions to ensure unifirmity in the given manner
        # we deep copy the list to avoid removing positions from self.legalPositions
        pool = list(permutatoins)

        # create an empty particles object
        self.particles = []
    ...
    def observeState(self, gameState):
        ...
        # deal with the edge case when one ghost have been captured
        # loop over the indeces of all ghosts that are dead
        for i in [x for x in range(self.numGhosts) if noisyDistances[x] == None]:
            # create the new particles with that specific ghost in jail
        ...
        for p in self.particles:
            ...
            # loop over ghosts that are not dead
            for i in [x for x in range(self.numGhosts) if noisyDistances[x] != None]:
                # update dists[p]
                dists[p] *= emissionModels[i][util.manhattanDistance(p[i], pacmanPosition)]


        # When all particles receive 0 weight, they should be recreated from the prior distribution
        if dists.totalCount() == 0.0:
            self.initializeParticles()
        else:
            # get the current belief distributions
            beliefDists = self.getBeliefDistribution()
            # loop over all particles
            for p in self.particles:
                # update the belief distributions then reset the local one
                beliefDists[p] *= dists[p]
                dists[p] = 1.0
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for i in range(self.numGhosts):
                ...
                # note that the new particles are used as ghost positions
                ghostsPos = setGhostPositions(gameState, newParticle)
                # get distributions over new positions for the i'th ghost
                newPosDist = getPositionDistributionForGhost(ghostsPos, i, self.ghostAgents[i])

                # update the entry in newParticle
